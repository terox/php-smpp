...
use OnlineCity\SMPP\Exception\SmppException;
...
class SmppClient
{
    ...
    /**
     * If we want show debug messages
     *
     * @var bool
     */
	protected $debug;
	 ...
	 */
	public function __construct(SocketTransport $transport, array $options = array(), $debug = false, $debugHandler = null)
	{
	    // Resolve options
	...
	public function configureOptions(OptionsResolver $resolver)
    {
        $resolver->setDefaults(array(
            'csmsMethod'                   => self::CSMS_16BIT_TAGS,
            ...
            'smsRegisteredDeliveryFlag'    => 0x00,
            'smsServiceType'               => '',
            'smsEsmClass'                  => 0x00,
            'smsProtocolId'                => 0x00,
            'smsPriorityFlag'              => 0x00,
            'smsReplaceIfPresentFlag'      => 0x00,
            'smsSmDefaultMsgId'            => 0x00,
            // Note: SMPP v3.4 says octect string are "not necessarily NULL terminated". Switch to toggle this feature.
            'smsNullTerminateOctetstrings' => true,
        ));
	...
	public function sendSMS(SmppAddress $from, SmppAddress $to, $message, $tags=null, $dataCoding=SMPP::DATA_CODING_DEFAULT, $priority=0x00, $scheduleDeliveryTime=null, $validityPeriod=null)
	{
		...
		if ($msg_length > $singleSmsOctetLimit) {
			$doCsms = true;
			if ($this->options['csmsMethod'] != SmppClient::CSMS_PAYLOAD) {
		...
		if ($doCsms) {
			...
			} else if ($this->options['csmsMethod'] == SmppClient::CSMS_8BIT_UDH) {
				$seqnum = 1;
				foreach ($parts as $part) {
					$udh = pack('cccccc',5,0,3,substr($csmsReference,1,1),count($parts),$seqnum);
					$res = $this->submit_sm($from, $to, $udh.$part, $tags, $dataCoding, $priority, $scheduleDeliveryTime, $validityPeriod, ($this->options['smsEsmClass']|0x40));
					$seqnum++;
				}
				return $res;
			} else {
	...
	{
		if (is_null($esmClass)) $esmClass = $this->options['smsEsmClass'];

		// Construct PDU with mandatory fields
		$pdu = pack('a1cca'.(strlen($source->value)+1).'cca'.(strlen($destination->value)+1).'ccc'.($scheduleDeliveryTime ? 'a16x' : 'a1').($validityPeriod ? 'a16x' : 'a1').'ccccca'.(strlen($short_message)+($this->options['smsNullTerminateOctetstrings'] ? 1 : 0)),
			$this->options['smsServiceType'],
			$source->ton,
			$source->npi,
			$source->value,
			$destination->ton,
			...
			$destination->value,
			$esmClass,
			$this->options['smsProtocolId'],
			$priority,
			$scheduleDeliveryTime,
			$validityPeriod,
			$this->options['smsRegisteredDeliveryFlag'],
			$this->options['smsReplaceIfPresentFlag'],
			$dataCoding,
			$this->options['smsSmDefaultMsgId'],
	...
	{
		$limit = ($this->options['csmsMethod'] == SmppClient::CSMS_8BIT_UDH) ? 255 : 65535;
	...
	protected function _bind($login, $pass, $command_id)
	{
		...
		$pduBody = pack(
			'a'.(strlen($login)+1).
			'a'.(strlen($pass)+1).
			'a'.(strlen($this->options['systemType'])+1).
			'CCCa'.(strlen($this->options['addressRange'])+1),
			$login, $pass, $this->options['systemType'],
            $this->options['interfaceVersion'], $this->options['addrTon'],
            $this->options['addrNpi'], $this->options['addressRange']
		);
